...
4. 最后剩余一个`BUG`，在截屏里面，是没有将头像获取下来，经查阅资料发现，是由于头像采用的mockjs模拟的，而项目代码又部署在另外一个服务器，因而出现跨越情况导致此情况出现，解决办法:在`html2canvas`中添加参数 ==> 允许跨域：allowTaint: true，服务器地址运行跨越，即设置 [`CORS`](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS)
